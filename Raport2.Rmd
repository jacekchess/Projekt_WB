...
The performance is checked using:

 **F1 Measure** -  is the harmonic mean of the precision and recall
...
f1_combined<-f1_combined_lda

ranking1<-function(f1_combined){
  ranks<-as.data.frame(lapply(f1_combined_ranger%>%select(-data_name), dense_rank))%>%
    mutate(data_name=f1_combined$data_name)
  
  ranks[is.na(ranks)]<-11
}
...
ggplot(rating_rpart, aes(x = variable, y = value)) +
  geom_boxplot() +
  geom_point(data = ranking_rpart[, c(1, 19)], aes(x = variable, y = value, color = 'red')) +
...
rating_nb <- reshape2::melt(f1_combined_nb)

ranking_nb<-ranking(f1_combined_nb)

ggplot(rating_nb, aes(x = variable, y = value)) +
  geom_boxplot() +
  geom_point(data = ranking_nb[, c(1, 19)], aes(x = variable, y = value, color = 'red')) +
  theme(axis.title.x = element_blank(),
        axis.title.y = element_blank())
...
rating_ranger <- reshape2::melt(f1_combined_ranger)

ranking_ranger<-ranking(f1_combined_ranger)

ggplot(rating_ranger, aes(x = variable, y = value)) +
  geom_boxplot() +
  geom_point(data = ranking_ranger[, c(1, 19)], aes(x = variable, y = value, color = 'red')) +
  theme(axis.title.x = element_blank(),
        axis.title.y = element_blank())
...
rating_lda <- reshape2::melt(f1_combined_lda)

ranking_lda<-ranking(f1_combined_lda)

ggplot(rating_lda, aes(x = variable, y = value)) +
  geom_boxplot() +
  geom_point(data = ranking_lda[, c(1, 19)], aes(x = variable, y = value, color = 'red')) +
  theme(axis.title.x = element_blank(),
        axis.title.y = element_blank())
...
rating_kknn <- reshape2::melt(f1_combined_kknn)

ranking_kknn<-ranking(f1_combined_kknn)

ggplot(rating_kknn, aes(x = variable, y = value)) +
  geom_boxplot() +
  geom_point(data = ranking_kknn[, c(1, 19)], aes(x = variable, y = value, color = 'red')) +
  theme(axis.title.x = element_blank(),
        axis.title.y = element_blank())
...
ranking_combined<-as.data.frame(rbind(
  ranking_rpart$ranks,
  ranking_nb$ranks,
  ranking_ranger$ranks,
  ranking_lda$ranks,
  ranking_kknn$ranks), row.names = c("rpart", "naiveBayes", "ranger", "LDA", "kknn"))
colnames(ranking_combined)<-ranking_rpart$variable
