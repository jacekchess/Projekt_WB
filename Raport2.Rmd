...
library(dplyr)
data_nr <- c("1590", "188", "27", "29", "38", "4", "40536", "55", "56", "6332", "944", "1018", "23381")
data_name <- c("adult", "eucalyptus", "colic", "credit-approval", "sick", "labor", "SpeedDating", "hepatitis", "vote", "cylinder-bands", "echoMonths", "ipums_la_99-small", "dresses-sales")
nr_instances <- c(48842, 736, 368, 690, 3772, 57, 8378, 155, 435, 540, 130, 8844, 500)
nr_features <- c(13, 16, 20, 16, 28, 17, 123, 20, 17, 34, 10, 57, 13)
nr_missing<-c(6465, 455, 1199, 67, 2293, 326, 18570, 167, 392, 999, 97, 34843, 835)
...
The overall measure of the imputation performance is a mean of it's ranks from all datasets.
...
# numery używanych obecnie: 1590, 188, 27, 29, 38, 4, 40536, 55, 56, 6332, 944
...
ranking<-function(f1_combined){
  ...
  ranks[is.na(ranks)]<-5
  ranks<-ranks%>%select(14, 1:13)
  colnames(ranks)<-c("imputation",data_name)
  
  ranks<-cbind(ranks, mean=rowMeans(ranks[2:14]))
...
f1_combined_ranger<-as.data.frame(
  ...
  evaluation_1018_ranger$f1,
  evaluation_23381_ranger$f1
))
...
f1_combined_kknn<-as.data.frame(
  ...
  evaluation_1018_kknn$f1,
  evaluation_23381_kknn$f1
))
