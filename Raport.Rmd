...
Replaces missing values with mean in numeric columns, or mode in character columns.
...
Method for handling missing data in which each missing value is replaced with an observed response from a “similar” unit.

### softImpute with median/mode imputation

Fits a low-rank matrix approximation to a matrix with missing values via nuclear-norm regularization.
...
```{r small data, cache=TRUE, warning=FALSE}
