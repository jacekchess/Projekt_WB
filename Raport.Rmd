...
```{r small data, cache=TRUE, warning=FALSE, message=FALSE, results='hide'}
...
```{r big data 1, cache=TRUE, warning=FALSE, message=FALSE,}
...
```{r big data 2, cache=TRUE, warning=FALSE, message=FALSE,}
evaluation_40536 <- evaluate_imputations(dataset40536,target40536) 
```

```{r big data 3, cache=TRUE, warning=FALSE, message=FALSE,}
# evaluation_23381 <- evaluate_imputations(dataset23381,target23381) # nie działa :( 
```

```{r big data 4, cache=TRUE, warning=FALSE, message=FALSE,}
...
```{r big data 5, cache=TRUE, warning=FALSE, message=FALSE,}
evaluation_1590 <- evaluate_imputations(dataset1590,target1590) 
```

```{r, results= 'markup', warning=FALSE, message=FALSE, include=FALSE}
...
```{r, results='markup'}
knitr::kable(naniar::miss_var_summary(dataset4))
...
In that situation, choosing a simpler method, such as mean insertion or choosing a random value for the column, in this exaple, showed to effect in better results.

## Number of missing values in datasets

Consider the following table of percent of values missing in a dataset, calculated as number of all cells with missing value, divided by number of all cells.

```{r missing-nr, results='markup'}
datasets1<-datasets%>%
  ...
  select(data_nr, data_name, procent_missing)%>%
  arrange(desc(procent_missing))

knitr::kable(datasets1)
```

As you can see, the dataset `labor` has by far the highest percent of the missing values, over double the percent of the next one in this ranking. If we take a closer look at the second one, `colic`, the balanced accuracy measure is also slightly higher in the mean insertion and hotdeck method, but this time softImpute did just as well as them.

```{r, results='markup'}
knitr::kable(bacc_combined[3,])
```
